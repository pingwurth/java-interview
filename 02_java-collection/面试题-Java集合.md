...
- **引入红黑树，优化极端性能**
  这是 1.8 中最核心的改变。当哈希冲突严重，导致某个桶（bucket）中的链表长度超过阈值 **8**，并且整个数组的长度大于等于 **64** 时，链表会转换为**红黑树**。这能确保在极端的哈希冲突下，查找、删除等操作的性能不会急剧下降。如果数组长度小于 64，即便链表超长，也会优先选择**扩容**来减少冲突。
- **修改插入顺序，解决死循环Bug**
  在 JDK1.7 中，新元素会采用**头插法**插入链表头部。这在多线程并发扩容时，容易使链表形成环，导致后续操作陷入**死循环**。JDK1.8 将其改为**尾插法**，新元素直接追加在链表末尾。这一改动有效避免了环形链表的产生，是修复该并发 Bug 的关键。
- **⚙️ 优化哈希函数，让散列更均匀**
  `HashMap` 在计算键（key）的存储位置前，会先对 `hashCode()` 进行“扰动”处理，以使散列更均匀。JDK1.8中的哈希函数 `hash()` 简化了运算过程，仅通过一次**高16位与低16位的异或运算** `(h = key.hashCode()) ^ (h >>> 16)`，就让高位的特征也能影响最终索引的计算结果。这种方式既高效又能有效减少冲突。
- **改进扩容机制，提升迁移效率**
  扩容时，JDK1.8 不再像 1.7 那样需要为每个元素重新计算哈希值。由于容量始终是 2 的幂次方，元素在新数组中的新位置只有两种可能：**原索引位置**，或 **“原索引 + 旧容量”位置**。这个结果直接取决于元素哈希值的新增的那个 bit 位是 0 还是1，因此可以通过简单的位运算快速判断，大大提高了 rehash 的效率

#### HashMap 在并发场景中有什么问题？

1、多线程 put 的时候，size 的个数和真正的个数不一样
2、多线程 put 的时候，可能会把上一个 put 的值覆盖掉
3、和其他不支持并发的集合一样，HashMap 也采用了 fail-fast 操作，当多个线程同时 put 和 get 的时候，会抛出并发异常
4、当既有 get 操作，又有扩容操作的时候，有可能数据刚好被扩容换了桶，导致 get 不到数据

> 1.7之前，扩容还可能导致死循环

#### ConcurrentHashMap 为什么在 JDK1.8 中废弃了分段锁？

​    虽然分段锁提高了并发性，但在段数固定的情况下，并发很高的时候仍可能导致热点段，从而成为性能瓶颈。另外，由于每个段都是独立的结构，这可能导致较高的内存占用。

> JDK 1.8 对 ConcurrentHashMap 的实现进行了重大改进，不再使用分段锁，而是采用了一种基于节点锁的方法，并且在内部大量使用了 CAS 操作来管理状态。这种新的设计旨在提供更高的并发级别并减少锁的争用。
>
...
并发冲突不高，synchronized 不会升级为重量级锁，性能不比 ReentrantLock 差。synchronized 还有其他的优势：

- synchronized 则是 JVM 内置的语义，JVM 能够在运行时作出相应的优化措施，比如锁粗化、锁消除等；
- 当获取锁获取失败时，synchronized 会通过自旋避免线程被挂起，而 ReentrantLock 会导致线程挂起，线程不需要挂起的话就可以减少线程上下文切换的开销；
- synchronized 是利用对象头（Object Header）中的一部分位标记来实现的锁，相比较 ReentrantLock，每次使用都需要实例化一个 ReentrantLock 对象，synchronized 的内存开销会更小。
