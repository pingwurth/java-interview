...
**JVM:** Java Virtual Machine, Java 虚拟机，用于运行 Java 程序，将字节码翻译为机器码，不同操作系统 JVM 版本不同，是 Java 实现跨平台语言的倚仗。

**JRE:** Java Runtime Environment, Java 运行时环境，运行已编译的 java 程序所需的组件，包含了 JVM。

**JDK:** Java Development Kit, Java 开发工具包，是一个功能齐全的软件开发工具包（SDK），包含 JRE、javac 编译工具。
...
基本数据类型：数值型（byte、short、int、long、float、double），布尔型，字符型
...
重载是编译时的多态，发生在一个类中，方法名相同，参数顺序、个数、类型不同。

重写时运行时的多态，发生在子类与父类，方法签名相同，子类的访问范围不能小于父类。

#### 局部变量与成员变量的区别？

局部变量时方法块中声明的，存储在栈帧的局部变量表，是线程安全的。

成员变量是在类中声明的，如果不是基本类型，就存在堆中；

成员变量如果没有显示赋值也有默认值，局部变量则不会有。
...
一般会先用 == 判断，相等返回 true，然后再判断类型，类型不相等返回 false，最后再比较对象的每一个属性值。String 类型是遍历 char 数组比较每个字符，包装类型是转换为基础类型用 == 比较。
...
> 【`==` 和 `equals`】
>
> - `==` 比较两个对象基于内存引用，如果两个对象的引用完全相同（指向同一个对象）时，`==` 操作将返回 true，否则返回 false。基本数据类型比较的是值。
>
...
>   
>  - 特例 `String`：直接使用双引号声明出来的 String 对象会直接存储在常量池中。当创建 String 类型的对象时，虚拟机会在常量池中查找有没有已经存在的值和要创建的值相同的对象，如果有就把它赋给当前引用。如果没有就在常量池中重新创建一个 String 对象。String 类的 equals 方法是重写过的，比较的是数值。
>   - 特例 `包装类`：使用 cache 数组缓存 -128 ~ 127 的值，这个范围内使用 == 和 equals 的效果是一样的，不过使用的时候只会用 equals。
...
其实， `hashCode()` 和 `equals()`都是用于比较两个对象是否相等。但是在一些容器（比如 `HashMap`、`HashSet`）中，有了 `hashCode()` 之后，判断元素是否在对应容器中的效率会更高。

hash 算法越好（数学问题），hash 碰撞越少。
...
`new String()` 在堆里创建一个 "abc" 字符串对象实例，而后者直接在字符串常量池里创建 "abc" 字符串常量并指向。

![创建字符串](./images/创建字符串.png)
...
![Object的方法](./images/Object的方法.png)
...
- Java 原生的，ObjectOutputStream 和 ObjectInputStream；

- Json 序列化，JSON.toJSONString 和 JSON.parseObject；

- ProtoBuf 序列化，一种轻便高效的结构化数据存储格式，ProtoBuff 序列化对象可以很大程度上将其压缩，可以大大减少数据传输大小，提高系统性能。

其他常见的有 Hessian、Kryo、ProtoStuff，这些都是基于二进制的序列化协议。
...
> 在执行 finally 之前，JVM 会先将 i 的结果暂存起来，然后 finally 执行完毕后，会返回之前暂存的结果，而不是返回 i
